...
class MATuning:
    ...
    def profit_GOOD(self,lasttime):
        ...
        x = self.profit_MX('select stkcode from stk_cb_t',sql,lasttime)
        sql = '''
          update mx_t set succ=%s,profit=%s,last_time=sysdate where code='mxjzyx'
            '''
    ...
    def ins_STKS(self,opt):
        ...
        sql = '''
          select stkcode,to_char(trdate,'mm/dd') trdate,0 pro,'1d'||seq_ml.nextval id
            from bs_t1 
            where trdate=to_date('%s','yyyymmdd') and status=1 and stkcode in (select stkcode from stk_cb_t)
            '''
        rows = self.cur.execute(sql % (y[1],)).fetchall()
        # rows = self.cur.execute(sql % ('20160711',)).fetchall()

        e = etl()
        e.setML('mxjzyx',rows,9) # 更新股票列表
        self.profit_GOOD(y[1])         
        e.setMX()
    
    def comp(self):
        sql = '''select to_char(sysdate-30-%s,'yyyymmdd'),to_char(sysdate-%s-30,'D') from dual'''
        for i in range(0,365):
            ...
            m = self.profit_DAY('select stkcode from stk_cb_t',now)
