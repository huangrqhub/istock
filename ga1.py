...
def MA_Cross(price_dt,short_ma=10,long_ma=20):
    ...
    s=ta.SMA(np.array(price_dt),timeperiod=short_ma)
    l=ta.SMA(np.array(price_dt),timeperiod=long_ma)
    signal=pd.DataFrame({'s':s,'l':l})
#    signal.loc[(signal['s']>signal['l']) & (signal['s'].shift()<signal['l'].shift()),'bs']=1
#    signal.loc[(signal['s']<signal['l']) & (signal['s'].shift()>signal['l'].shift()),'bs']=0
...
def indic_cal(price_data,indic_dict):
    ...
    for k in indic_dict:
        ...
        if k=='ma_cross':
            price_data['ma_cross_SIGNAL']=chromo_fun['ma_cross'](price_dt=np.array(price_data['CL'],dtype='f8'),short_ma=opt_ks,long_ma=opt_kl)
        if k=='rsi':
        ...
        if k=='ema_ali':
            price_data['ema_ali']=chromo_fun['ema_ali'](real=np.array(price_data['CL'],dtype='f8'))
        ...
        if k=='kdj':
            price_data['kdj_slowk'],price_data['kdj_slowd']=chmoro_fun['kdj'](np.array(price_data['HI'],dtype='f8'),np.array(price_data['LO'],dtype='f8'),np.array(price_data['CL'],dtype='f8'))
    ...
    signal=0
    for k in indic_dict:
        signal+=price_data[k+'_SIGNAL'].iloc[l-1]*indic_dict[k]['w']
    if signal>0.5:
        return 1
    else:
        return 0
...
def price_get(stock_code='300109'):
    ...
    # sql = '''
    #     select trdate,op*(exb/cl) op,hi*(exb/cl) hi,lo*(exb/cl) lo,exb cl,ch/100 ch 
    #       from mv_stk_td where stkcode='%s' order by trdate
    #     '''
    sql = '''
        select trdate,v_open*(adj_price/v_close) op,v_high*(adj_price/v_close) hi,v_low*(adj_price/v_close) lo,adj_price cl,v_change ch
...
class indic_w(object):
    ...
    def para_opt(self,stock_code,period=[range(10,16)+range(45,61)]):
        ...
        for ps in range(7,120,7):
            pl=ps+7
            while(pl<140):
                data['ma_cross_SIGNAL']=MA_Cross(price_dt=data['CL'],short_ma=ps,long_ma=pl)
                if 'deal' in data.columns : del data['deal']
                data.loc[data[data[k+'_SIGNAL']>data[k+'_SIGNAL'].shift()].index,'deal']=1
                data.loc[data[data[k+'_SIGNAL']<data[k+'_SIGNAL'].shift()].index,'deal']=0
                ep=get_stat(data)[0]
...
#            self.opt_prof.append([opt_p,'ma_cross'])
        ...
        if opt_p==0:
            ...
            self.opt_para['ma_cross']={'ma_cross_sma':opt_ks,'ma_cross_lma':opt_kl}
            self.opt_prof.append([opt_p,'ma_cross'])
            self.data_signal['ma_cross_SIGNAL']=chromo_fun['ma_cross'](price_dt=np.array(self.data_signal['CL'],dtype='f8'),short_ma=opt_ks,long_ma=opt_kl)
...
#        a['STOCKCODE'][sci]='300344'
    ...
    c = mdb.ga
    print('result',res)
    print 'stock',sci,', time:',(datetime.now()-t).seconds
    c.save(res)
